...
@pytest.mark.parametrize("collect_arg", ["systemd-unit-status", "systemd-unit-logs"])
@pytest.mark.parametrize(
    "send_arg",
    ["alert-manager", "file-output", "logged-in-users", "stream-output"],
)
def test_systemd_module(tmp_path, collect_arg, send_arg):
    runner = CliRunner(mix_stderr=False)

    with runner.isolated_filesystem(temp_dir=tmp_path) as d:
        temp_dir = pathlib.Path(d)
        collect_available = {
            "systemd-unit-status": ["--name", "testing"],
            "systemd-unit-logs": ["--name", "testing"],
        }
